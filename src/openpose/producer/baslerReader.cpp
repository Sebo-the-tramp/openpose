...
namespace op
{
    BaslerReader::BaslerReader(const std::string& cameraParameterPath, const Point<int>& cameraResolution,
                           const bool undistortImage, const int cameraIndex) :
        Producer{ProducerType::NdiCamera, cameraParameterPath, undistortImage, -1},
        mBaslerWrapper{cameraParameterPath, cameraResolution, undistortImage, cameraIndex},
        mFrameNameCounter{0ull}
    {
        try
        {
            // Get resolution
            const auto resolution = mBaslerWrapper.getResolution();                        
    ...
    std::vector<Matrix> BaslerReader::getCameraMatrices()
    {
        try
        {
            return mBaslerWrapper.getCameraMatrices();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    std::vector<Matrix> BaslerReader::getCameraExtrinsics()
    {
        try
        {
            return mBaslerWrapper.getCameraExtrinsics();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    std::vector<Matrix> BaslerReader::getCameraIntrinsics()
    {
        try
        {
            return mBaslerWrapper.getCameraIntrinsics();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    std::string BaslerReader::getNextFrameName()
    {
        try
        {
    ...
    bool BaslerReader::isOpened() const
    {
        try
        {
            return mBaslerWrapper.isOpened();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    void BaslerReader::release()
    {
        try
        {
            mBaslerWrapper.release();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    Matrix BaslerReader::getRawFrame()
    {
        try
        {
            return mBaslerWrapper.getRawFrames().at(0);
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    std::vector<Matrix> BaslerReader::getRawFrames()
    {
        try
        {
            mFrameNameCounter++; // Simple counter: 0,1,2,3,...
            return mBaslerWrapper.getRawFrames();
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
    ...
    double BaslerReader::get(const int capProperty)
    {
        try
        {
    ...
    void BaslerReader::set(const int capProperty, const double value)
    {
        try
        {
            if (capProperty == CV_CAP_PROP_FRAME_WIDTH)
